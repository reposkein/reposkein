...
- **The camera moves only when it has to** — selecting or hopping to a star you can already see leaves the view exactly where it is; the camera flies only when the target is off screen. Pressing `x` repeatedly climbs out one level at a time and stops at the top level rather than snapping to a reset. `Esc` backs out one step of *attention* (overlay → layer → mode → selection) and never rearranges the graph.
