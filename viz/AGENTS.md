...
- **Fonts are self-hosted**, referenced relatively from `src/styles/fonts.css`, so the static export has zero external requests over `file://`. Never add a CDN font or stylesheet.
